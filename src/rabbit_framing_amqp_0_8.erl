...
-type(amqp_method_field_name() ::
       ( routing_key | routing_key | routing_key | global
       | channel_max | consumer_tag | frame_max | heartbeat
       | dtx_identifier | nowait | delivery_tag | virtual_host
       | queue | capabilities | mandatory | reply_text
       | insist | ticket | identifier | integer_1
       | consumer_tag | known_hosts | prefetch_count | prefetch_size
       | host | arguments | version_minor | known_hosts
       | staged_size | reply_code | reply_text | queue
       | class_id | cluster_id | method_id | exclusive
       | no_ack | consumer_tag | exclusive | consumer_tag
       | no_local | no_ack | routing_key | out_of_band
       | exchange | nowait | nowait | exclusive
       | passive | active | consumer_tag | routing_key
       | active | delivery_tag | mandatory | exchange
       | result | reply_code | reply_text | delivery_tag
       | details | locales | identifier | exchange
       | reply_code | reply_text | routing_key | class_id
       | consumer_tag | method_id | redelivered | reply_text
       | exchange | exchange | consume_rate | reply_text
       | routing_key | delivery_tag | version_major | realm
       | integer_result | server_properties | content_size | mechanisms
       | table | requeue | exchange | active
       | client_properties | nowait | write | read
       | mechanism | requeue | response | locale
       | routing_key | routing_key | consumer_tag | consumer_tag
       | ticket | immediate | exchange | response
       | identifier | passive | durable | ticket
       | reply_code | auto_delete | ticket | delivery_tag
       | internal | nowait | arguments | multiple
       | string_2 | prefetch_size | prefetch_size | operation
       | consumer_tag | ticket | exchange | string_op
       | if_unused | nowait | requeue | message_count
       | nowait | consumer_tag | reply_code | delivery_tag
       | string_result | integer_op | operation | ticket
       | queue | consumer_tag | passive | durable
       | global | queue | challenge | exclusive
       | immediate | auto_delete | nowait | arguments
       | reply_code | requeue | exchange | queue
       | message_count | redelivered | result | integer_2
       | consumer_count | no_local | redelivered | exclusive
       | ticket | queue | prefetch_count | type
       | exchange | consumer_tag | routing_key | nowait
       | arguments | consumer_tag | nowait | delivery_tag
       | routing_key | ticket | ticket | queue
       | nowait | no_ack | consumer_tag | ticket
       | multiple | channel_max | no_local | ticket
       | message_count | integer_4 | mandatory | immediate
       | queue | ticket | meta_data | queue
       | string_1 | ticket | if_unused | frame_max
       | ticket | if_empty | nowait | exchange
       | prefetch_count | exchange | global | content_checksum
       | message_count | queue | heartbeat | consumer_tag
       | exchange | integer_3 | delivery_tag | ticket
       | queue | exchange )).
