...
-type(amqp_method_field_name() ::
       ( nowait | ticket | nowait | queue
       | passive | durable | exclusive | auto_delete
       | client_properties | prefetch_count | nowait | arguments
       | delivery_tag | multiple | queue | response
       | message_count | consumer_tag | consumer_count | version_major
       | version_minor | ticket | queue | locales
       | delivery_tag | cluster_id | routing_key | message_count
       | nowait | arguments | response | locale
       | requeue | redelivered | challenge | ticket
       | queue | nowait | channel_max | reply_text
       | frame_max | consumer_tag | heartbeat | exchange
       | consumer_tag | available | ticket | channel_max
       | frame_max | heartbeat | if_unused | delivery_tag
       | ticket | virtual_host | capabilities | insist
       | known_hosts | consumer_tag | ticket | reply_code
       | queue | reply_text | class_id | credit
       | method_id | routing_key | arguments | queue
       | exchange | reason | nowait | out_of_band
       | exchange | channel_id | ticket | prefetch_size
       | active | consumer_tag | credit_drained | global
       | mechanism | requeue | active | queue
       | routing_key | reply_code | reply_text | class_id
       | method_id | consumer_tag | mandatory | no_local
       | no_ack | no_ack | realm | routing_key
       | exclusive | passive | active | exclusive
       | write | message_count | read | ticket
       | consumer_tag | nowait | ticket | drain
       | exchange | requeue | type | passive
       | durable | auto_delete | arguments | delivery_tag
       | internal | nowait | delivery_tag | arguments
       | immediate | ticket | ticket | exchange
       | if_unused | nowait | routing_key | ticket
       | server_properties | destination | queue | source
       | routing_key | nowait | arguments | multiple
       | exchange | mechanisms | ticket | destination
       | source | routing_key | requeue | nowait
       | exchange | arguments | if_empty | redelivered
       | reply_code | exchange | routing_key | message_count )).
